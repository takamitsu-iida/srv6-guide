...
- Next Header: IPIP (4) となっています。ペイロードはIPv4パケットであることを意味しています。

- Source: 2001:db8:0:3::1 となっています。これはPE03のループバックです。そうなるように設定しています。

- Destination: 2001:db8:0:4:42:: となっています。これはPE04のロケータ2001:db8:0:4::/64から払い出されたSIDで、ファンクション部は0x42です。

<br>

## CR01の経路

CR01はルートリフレクタ役を担っていますので、全てのPEとiBGPで接続しています。
PE03とPE04からそれぞれ経路を学習しています。
nexthopはPE03とPE04のループバックアドレスです。
...
`2001:db8:0:3::43::` の行に注目。

'default' というvrfが最初から存在していて、そのvrfに対するEnd.DT4になっています。
...
インタフェースに設定するMTUは、9000バイトにイーサネットヘッダ14バイトを加えて9014としています。
同じ意味合いでCSR1000vに設定するときにはMTUを9000バイトと設定します。
IOS-XRとIOS-XEではMTUの数え方が異なるので注意が必要です。
...
eBGPでCE05と接続しますので、経路フィルタの設定が必須です。
IOS-XRはeBGPのネイバーに対してフィルタを設定しないと有効になりません。
...
BGPで学習したIPv4経路をSRv6に乗せるのは、この設定です。この設定がないと、IPv4のまま通信しようとします。
