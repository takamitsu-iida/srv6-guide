...
SRv6でイーサネットフレームを運ぶ場合の外側IPv6ヘッダのNext Headerは143です。

> **Note**
>
> 古い実装ではイーサネットフレームを運ぶ場合のNext Headerに59(IPv6-NoNxt)を使っているかもしれません。
> IOS-XRは`segment-routing srv6 encapsulation evpn next-header` 設定でこの値を変更できます）。
>
> 参照
>
...
![fig_3_2_4](img/fig_3_2_4.drawio.png)

ネットワークを論理的に分割したい、という要件はSRv6とFlexAlgoを利用することで実現できます。
FlexAlgoはメトリック（コスト値）以外のパラメータで経路を計算するものです。
複数のアルゴリズムで経路計算することからフレキシブルアルゴリズム（Flexible Algorithm, FlexAlgo）と呼ばれています。
SRv6で割り当てるセグメントID（SID）ごとにアルゴリズムを変えることで、SRv6網の中の通り道が変わります。
これによって論理的にネットワークを分離します。これをスライシングと呼びます。
...
[IOS-XRの設定例](../iosxr_config/README.md)を参照してください。
...
セグメントルーティングではルータが持つ機能に対してSIDを採番するわけですが、SRv6におけるSIDはIPv6アドレスと同じ形式を取ります。

IPv6アドレスは前半64ビットをネットワーク部、後半64ビットをホスト部と呼びます。
一方、SRv6では前半をLocator（ロケータ）、後半をFunction（ファンクション）と呼びます。
...
40ビットというのは16の倍数ではないのでIPv6のアドレス表記と相性が悪いのですが、そう決まっているので仕方ありません。
ブロック部はSRv6を構成するネットワーク内で共通にします。
...
>
> /48というのは、プロバイダーからIPv6アドレスの払い出しを受ける際の最大サイズです。
...
先頭の40ビットはブロック部になりますので、SRv6を構成する全てのルータで共通にしなければいけません。
ISPからグローバルアドレスを/48で払い出されたなら、このルールはすでに満たしています。
ユニークローカルアドレスfc00::/7を使う場合はこの制約に十分注意してください。

この例では 2001:0db8:00 がブロック部になります（16進表記のXXが5個で40ビットです）。

ノード部はルータごとに変わります。ノード部を1から連番で割り当てるなら、各装置のロケータはこうなります。

- R1のロケータは 2001:0db8:00 + 00:0001 = `2001:db8:0:1::/64`
- R2のロケータは 2001:0db8:00 + 00:0002 = `2001:db8:0:2::/64`
- R2のロケータは 2001:0db8:00 + 00:0003 = `2001:db8:0:3::/64`
- R2のロケータは 2001:0db8:00 + 00:0004 = `2001:db8:0:4::/64`

![fig_6](img/fig_6.drawio.png)

ルータにロケータを割り当てる、というのは、ルータに論理的なインタフェースを作成して/64のアドレスを割り当てるようなイメージになります。
...
[RFC8986](https://datatracker.ietf.org/doc/rfc8986/) で標準化されていますので、ここからはその呼び方を使います。

| ファンクション     |  説明  |
| ----------------- | ----- |
| End               | エンドポイント<br>そのルータ自身への通信を処理する機能です |
...
たくさんありますが、よく目にするファンクションはこれらです。順番に見ていきましょう。
...
ルータは最低でも２つの機能を持っています。一つは自分宛の通信を処理する機能です。

IPネットワークを設計するときのベストプラクティスでは、すべてのルータにループバックインタフェースを作成して、装置を代表するIPアドレスをそのループバックに割り当てます。
IPv4であれば/32、IPv6であれば/128のアドレスをループバックに割り当て、動的ルーティングでその情報を配信します。

たとえば、2001:db8:ffff:ffff::/64を管理用のサブネットにするのであれば、この中から各ルータに/128のアドレスを払い出して、OSPFやISISを使ってこの経路情報を配信します。

- PE01のLoopback0は 2001:db8:ffff:ffff::**1**/128
- PE02のLoopback0は 2001:db8:ffff:ffff::**2**/128
- PE03のLoopback0は 2001:db8:ffff:ffff::**3**/128
- PE04のLoopback0は 2001:db8:ffff:ffff::**4**/128

![node_sid_1](img/node_sid_1.drawio.png)

このアドレスはプロトコルのID、たとえばbgpやOSPFのルータIDとして使われるほか、
実際の通信、たとえばiBGPでピアリングするときのアドレスや、SYSLOGやSNMP等の管理通信の送信元アドレスとして利用されます。

<br>

SRv6では自装置宛の通信を処理する機能をEndと定義しています。
Endに割り当てるSIDのことをノードSID(Node-SID)と呼びます。
SRv6において「あの装置を経由せよ」と指示するときにノードSIDを指定します。

ノードSIDはロケータが決まると自動で採番されます。
IOS-XRではファンクション部が1、すなわち `{locator}:1::` がノードSIDとして割り当てられます。

装置を代表するIPv6アドレスと、装置を代表するSIDは、関連付けておくとわかりやすくなります。

- 装置を代表するアドレス = ループバックのIPアドレス = SRv6パケットの送信元アドレス

という関係になりますので、ループバックのIPアドレスはロケータと関連付けられていたほうがわかりやすくなります。

SRv6を構成するルータのLoopbackのIPv6アドレスは `{locator}::1/128` とする、といったように設計ルールにしておくとよいと思います。この設計ルールであれば、ロケータとループバックとnode-SIDはこの図のような関係になります。

![node_sid_2](img/node_sid_2.drawio.png)

たとえばロケータが 2001:0db8:0:1::/64 であれば、ループバックにはその中から 2001:0db8:0:1::1/128 というアドレスを割り当てます。ISISはSRv6のロケータ情報を/64の経路として配信しますので、ループバックに割り当てた/128のアドレスをconnected経路として再配送する必要はありません。

<br>

> **Note**
>
> ルータには複数のロケータをもたせられますが、SRv6パケットとしてカプセル化するときの送信元アドレスに指定できるのは1つだけです。

<br>

### End.X

ルータが必ず持つもう一つの機能は、自足のインタフェースの先にいるノードにパケットを中継する機能です。
すべてのルータがこの動作をすることで、ホップバイホップでパケットが中継されていくわけですが、SRv6ではこの機能をEnd.Xと定義しています。
もちろんSIDを採番する対象で、ノードが持っている足の数だけEnd.XのSIDを割り当てます。これを隣接関係SID（Ajacency SID、Adj-SID）と呼びます。
SRv6において「あのインタフェースを経由せよ」と指示するときには、隣接関係SIDを指定します。

隣接ノードを見つけるのはISISの仕事です。
ISISは隣接ノードを見つけ次第、ロケータの中からEnd.XのSIDを採番します。
End.Xの情報を詳細に調べると、そのインタフェースの先にいる隣接機器のIPv6アドレスを知ることができます。

動的に決めたEnd.XのSID(adj-SID)は装置の再起動やインタフェースのダウン・アップで変わってしまうかもしれません。
このインタフェースを通れ、と指定する場面では、事前にEnd.XのSIDが決まっていたほうがよいので、静的に設定した方がよいかもしれません。

<br>

> **Note**
>
> EndとEnd.Xはどちらもそのノード自身のSIDです。ざっくり、Endは装置監視の宛先として、End.Xはインタフェース監視の宛先として使う、と考えていいと思います。

<br>

## End.DT4とEnd.DT6、End.DX4とEnd.DX6

ルータには上記２つの機能EndとEnd.Xに加えて、VPNを収容することもあります。
...
ネットワーク設計によっては同じVRFの先に複数のCEルータがつながるケースがあります。
VRF単位にSIDを割り当てるのがEnd.DT、CE装置単位にSIDを割り当てるのがEnd.DXです。
...
自装置のVRFに対して割り当てたSIDの情報(End.DTやEnd.DX)は、iBGPを使って自分以外のエッジルータに配信します。
そのVRF宛ての通信は（iBGPのNext Hopアドレスではなく）配信したSIDに戻ってきます。
...
> VPNの通信に対して特別な扱いをしたい場合は、ISISのFlexAlgoを使うか、トラフィックエンジニアリングポリシーを適用する方が現実的です。
...
コマンドで`show route srv6`などと打ちたくなるわけですが、あいにくそのようなコマンドは存在しません。
SRv6はアドレス体系であって、情報を交換するプロトコルではありませんので、SIDのルーティングテーブルというものは存在しません。
End.Xの詳細情報はISISに、End.DTやEnd.DXはBGPの中に詳細情報が格納されています。
各プロトコルに関してshowコマンドを駆使して調べることになります。
...
>
> show bgp vpnv4 unicast [local-sids | received-sids]
>
...
これがSRv6の特徴の一つです。
MPLS VPNのときには一段目のラベルとして出口になるPE（より正確には出口PEのループバックアドレスに対応するラベル）が使われていました。
...
自分を目掛けて飛んできたIPv6パケットの宛先アドレスが自身のSIDであれば、SRv6としての処理を行います。
...
実のところ、SRヘッダは無い方が好ましいです。どこを経由せよ、と指定してもSRヘッダを使わなくてすむのが、この後紹介するマイクロSIDです。
...
「あのSIDにたどり着くためには、ここを通過せよ」という設定をします。
ただし、これは片道なので戻りの通信も考慮しなければいけません。現実には使いづらいです。

<br>

FlexAlgoを使ってISISの経路計算を論理的に分割する方法もあります。
ISISのメトリック（コスト値）で経路を計算するとコスト値に関しての最短パスが求まるわけですが、回線の遅延時間で計算すると違う経路の方が優れているかもしれません。
...
その経路をどうやって物理的に分離しておくか、を考える方が難しいことです。
...
> **Note** ココ重要
>
> 重要なので繰り返しますが、パケットの送信先はiBGPのnexthopではなく、SIDです。
...
> **Note**
>
> uSIDではRFC4193 ユニークローカルアドレス FC00::/8 の利用が推奨されます。
...
> **Note**
>
> F3216形式のuSIDであれば一つの宛先に6個まで経由地を指定できます。
> 7個以上の場合にSRヘッダが付きます。
