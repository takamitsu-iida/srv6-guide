...
SRv6において「あの装置を経由せよ」と指示するときには、ノードSIDを指定します。
...
- 装置を代表するアドレス = ループバックのIPアドレス
- 装置を代表するSID = SRv6のEnd機能に対するSID = ノードSID

たとえばロケータが 2001:0db8:0:1::/64 であれば、ループバックにはその中から 2001:0db8:0:1::1/128 というアドレスを割り当てます。

<br>

> **Note**
>
> SRv6を構成するルータのLoopbackのIPv6アドレスは {locator}::1/128 とする、といったように設計ルールにしておくとよいと思います。
